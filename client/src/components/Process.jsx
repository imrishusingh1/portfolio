...
const fallbackSteps = [
  ...
  { num: '02', title: 'Design', desc: 'Wireframes and prototypes iterated until satisfied.', icon: '💻', bg: '#ffe3f5' },
  { num: '03', title: 'Development', desc: 'Clean, scalable code with smooth animations.', icon: '🎨', bg: '#ffebe3' },
]
const fallbackTools = ['React', 'Node.js', 'Express', 'MongoDB', 'TypeScript', 'WebRTC', 'Redis', 'Docker', 'AWS', 'Next.js']

export default function Process() {
  const ref = useRef(null)
  const inView = useInView(ref, { once: true, margin: '-80px' })
  const { data } = useSectionData('process')
            ...
            {steps.map(({ num, title, desc, icon, bg }, i) => (
              ...
              >
                <div className="process-card-top">
                  <div className="process-icon-circle"><span>{icon}</span></div>
